...
The **shift chart** visualizes the selected states and the percentage shift in the outcome of selected states over the years(extra credit). For instance, you will be visualizing information such as: 'Ohio' has seen a shift of 11% towards Republican in the year 1980". 
...
In addition, the party that win the state votes will display in the column `State_Winner`. If it's empty, that means the state split the votes to two parties. We also take care this condition in the electoral votes chart and the tiles chart.
...
In the ``update`` method, create a stacked bar chart for each party where each bar represents single state. Unlike the bar charts that you have used before, this will have constant values for the *y* attribute and *height* and the appropriate scale is applied to the *x* attribute and *width*. 
...
Moreover, if a state splits the votes to two parties, we need to split the tile area by the votes ratio. And also color it based on correct `RD_Difference` after considering the vote ratio.
...
```
The values that you give in the extent will be the bounds for the chart on which you are visualizing. Next, 
...
Implement a brush on the year chart to select a range of election years. Use the data extracted from the brush to update your Voter Shift Visualization. If you're not doing the extra credit 2 part, just display the selected years in your webpage. If you're doing extra credit 2, make your visualization react to the selection of years.
...
Your final solution should behave something [like this](https://youtu.be/VpuXaajgJq4).  Note that we're not showing the shift chart, as we don't want to prime you with a design.
